...
describe("registerSiteTools", () => {
  ...
  it("should register all site tools", async () => {
    const result = await client.listTools();
    const toolNames = result.tools.map((t) => t.name);
    expect(toolNames).toContain("site_list");
    expect(toolNames).toContain("site_get");
    expect(toolNames).toContain("site_create");
    expect(toolNames).toContain("site_update");
  });

  it("should include descriptions for each tool", async () => {
    const result = await client.listTools();
    const ourTools = result.tools.filter(t => t.name.startsWith("site_"));
