...
9. **门禁分级执行**：定稿方案须包含 **分级验收门禁表**（建议四级：代码质量与审查 → 安全审查 → 单测+覆盖率门禁 → E2E 验收），每级写明验证方式、通过条件、责任人。**禁止**无门禁定义的方案进入实现阶段。
...
- **分级验收门禁表**（四级：代码质量与审查 → 安全审查 → 测试覆盖率门禁 → E2E 验收）
...
- **E2E 用例枚举**：每场景一张表（前置条件、逐条用例操作与断言、清理方案）
