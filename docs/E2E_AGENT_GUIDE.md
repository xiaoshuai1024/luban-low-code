...
| **BFF E2E** | 🔴 无 e2e（仅 Vitest unit）；契约由跨项目流程间接覆盖 | 不建独立浏览器 e2e | `cd apps/bff && pnpm run test`（unit） |
| **后端 Java 集成测** | JUnit+H2 `src/test/java/**/{*Test,*IT}.java` | 同现状 | `cd apps/backend-java && mvn -q verify` |
| **后端 Go 测试** | go test `**/*_test.go`（全单测） | 同现状 | `cd apps/backend-go && go test ./...` |
...
- 后端 API 须健康（Java 后端 `http://127.0.0.1:8080`，Go 后端按其配置）。**Agent 执行引擎 E2E 前**：若健康检查失败，**须尝试**启动后端：
  - Java：`cd apps/backend-java && mvn spring-boot:run`
  - Go：`cd apps/backend-go && go run`
- BFF 须健康（`cd apps/bff && pnpm run dev`）
- 引擎调试页或 website 须可访问
