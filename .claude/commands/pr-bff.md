---
description: 提交 apps/bff 子模块改动并创建 PR 到默认分支（只产生 PR，禁止直接推送默认分支）
---

对应子模块路径：`apps/bff`。在主仓库根目录执行：
...
**你必须：** 在主仓库根目录运行；确认 **apps/bff** 子模块当前分支为 `feature/*`、`bugfix/*` 或 `hotfix/*`（禁止在默认分支 master 上操作）；**禁止切换分支**；执行后输出 PR 结果。
