...
// pageColumns 显式列名（V2-T2 关键：禁 SELECT *，避免新增 seo_json 列后
// 列顺序敏感导致 json.RawMessage 解析错位）。
const pageColumns = `id, site_id, name, path, status, schema_json, seo_json, created_at, updated_at`
...
func (r *PageRepository) ListBySite(ctx context.Context, siteID string) ([]*model.Page, error) {
	...
	if err := r.db.SelectContext(ctx, &pages, `SELECT `+pageColumns+` FROM pages WHERE site_id = ? ORDER BY updated_at DESC`, siteID); err != nil {
		return nil, err
	}
	return pages, nil
}
...
func (r *PageRepository) Get(ctx context.Context, siteID, pageID string) (*model.Page, error) {
	var p model.Page
	err := r.db.GetContext(ctx, &p, `SELECT `+pageColumns+` FROM pages WHERE id = ? AND site_id = ?`, pageID, siteID)
	if err != nil {
...
func (r *PageRepository) GetPublishedBySiteAndPath(ctx context.Context, siteID, path string) (*model.Page, error) {
	var p model.Page
	err := r.db.GetContext(ctx, &p, `SELECT `+pageColumns+` FROM pages WHERE site_id = ? AND path = ? AND status = 'published'`, siteID, path)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, ErrPageNotFound
		}
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		p.ID, p.SiteID, p.Name, p.Path, p.Status, p.Schema, nullableJSON(p.Seo), p.CreatedAt, p.UpdatedAt,
	)
	if err != nil {
...
UPDATE pages SET name=?, path=?, status=?, schema_json=?, seo_json=?, updated_at=? WHERE id=? AND site_id=?`,
		p.Name, p.Path, p.Status, p.Schema, nullableJSON(p.Seo), p.UpdatedAt, p.ID, p.SiteID,
	)
	if err != nil {
		if strings.Contains(err.Error(), "Duplicate entry") {
			return ErrPagePathConflict
