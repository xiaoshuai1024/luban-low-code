-- 留资表单 + 销售线索表（P0 留资闭环）
--
-- 背景：forms/leads 表的 DDL 此前仅存在于已废弃的 schema.sql（参考用），
-- 未进入 Flyway 迁移，导致 FormMapper/LeadMapper 运行时报
-- "Table 'luban.forms' doesn't exist"。本迁移补齐这两张表，
-- DDL 与 schema.sql / luban-backend-go dao/mysql.go 对齐。
