---
description: 提交 apps/engine 子模块改动并创建 PR 到默认分支（只产生 PR，禁止直接推送默认分支）
---

对应子模块路径：`apps/engine`。在主仓库根目录执行：
...
1. 在**主仓库根目录**执行命令。
2. 确认 **apps/engine** 子模块当前分支为 `feature/*`、`bugfix/*` 或 `hotfix/*`（禁止在默认分支 master 上操作）。
